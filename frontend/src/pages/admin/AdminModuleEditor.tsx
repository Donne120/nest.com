...
function Section({ title, children, action }: { title: string; children: React.ReactNode; action?: React.ReactNode }) {
  return (
    <div className="bg-[var(--c-surf)] rounded-xl border border-[var(--c-rule)] overflow-hidden mb-5">
      <div className="flex items-center justify-between px-5 py-3.5 border-b border-[var(--c-rule)] bg-[var(--c-bg2)]/40">
...
const inputCls = "w-full border border-[var(--c-rule)] rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent transition";
  ...
  return (
    <div className="p-6 lg:p-8 max-w-3xl mx-auto">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-[var(--c-ink3)] mt-1">
      ...
      <Section title="Module Details">
        <div className="space-y-5">
          ...
          <div>
            <label className="block text-sm font-semibold text-[var(--c-ink2)] mb-1.5">
              Description
              <span className="ml-1.5 text-[11px] font-normal text-[var(--c-ink3)] bg-[var(--c-bg2)] px-2 py-0.5 rounded-full">
                Rich text supported
              </span>
            </label>
            <RichTextEditor
          ...
          <div>
            <label className="block text-sm font-semibold text-[var(--c-ink2)] mb-1.5">
              AI Context Notes
              <span className="ml-1.5 text-[11px] font-normal text-indigo-500 bg-indigo-500/10 px-2 py-0.5 rounded-full">
                Only visible to AI
              </span>
            </label>
            <textarea
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              />
              {thumbnailUrl && (
                <img src={thumbnailUrl} alt="Thumbnail" className="mt-2 h-16 w-full object-cover rounded-lg border border-[var(--c-rule)]" onError={() => {}} />
              )}
            </div>
            <div>
              <label className="block text-sm font-semibold text-[var(--c-ink2)] mb-1.5">Order Index</label>
              <input
                type="number" min={0}
                value={orderIndex}
          ...
          <label className="flex items-center gap-3 cursor-pointer select-none">
            <div className="relative flex-shrink-0">
              ...
              <div className="w-10 h-6 bg-[var(--c-rule)] rounded-full peer peer-checked:bg-brand-600 transition-colors" />
              <div className="absolute top-1 left-1 w-4 h-4 bg-[var(--c-surf)] rounded-full shadow transition-transform peer-checked:translate-x-4" />
            </div>
            <div>
              <p className="text-sm font-semibold text-[var(--c-ink2)]">Published</p>
        ...
        <div className="mt-6 pt-4 border-t border-[var(--c-rule)] flex justify-end">
        ...
        <div className="space-y-4">
          ...
          <label className="flex items-center gap-3 cursor-pointer select-none">
            <div className="relative flex-shrink-0">
              ...
              <div className="w-10 h-6 bg-[var(--c-rule)] rounded-full peer peer-checked:bg-emerald-500 transition-colors" />
              <div className="absolute top-1 left-1 w-4 h-4 bg-[var(--c-surf)] rounded-full shadow transition-transform peer-checked:translate-x-4" />
            </div>
            <div>
              <p className="text-sm font-semibold text-[var(--c-ink2)]">Sell this module separately</p>
              <p className="text-xs text-[var(--c-ink3)]">
          ...
          {isForSale && (
            <div className="flex gap-3 items-end pl-[52px]">
              <div className="flex-1">
                <label className="block text-xs font-semibold text-[var(--c-ink2)] mb-1.5">Price</label>
                <input
                  type="number"
                  min={0}
                  step="any"
              ...
              <div className="w-28">
                <label className="block text-xs font-semibold text-[var(--c-ink2)] mb-1.5">Currency</label>
                <select
                  value={currency}
          ...
          {isForSale && price && (
            <div className="ml-[52px] flex items-center gap-2 text-xs text-emerald-400 bg-emerald-500/10 border border-emerald-500/25 rounded-lg px-3 py-2">
              <span>✓</span>
      ...
      <Section
        ...
        action={
          <button
            type="button"
            onClick={addResource}
            className="flex items-center gap-1.5 text-xs font-semibold text-brand-600 hover:text-brand-400 transition-colors"
          >
      ...
      >
        {resources.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-8 text-center">
            <div className="w-10 h-10 bg-[var(--c-bg2)] rounded-xl flex items-center justify-center mb-3">
              <LinkIcon size={18} className="text-[var(--c-ink3)]" />
            </div>
            <p className="text-sm font-medium text-[var(--c-ink2)]">No resources yet</p>
            <p className="text-xs text-[var(--c-ink3)] mt-1 mb-3">
              Add docs, links, PDFs or videos for learners to reference
            </p>
            <button
              type="button"
              onClick={addResource}
              className="text-xs font-semibold text-brand-600 hover:text-brand-400 transition-colors"
            >
              + Add first resource
            </button>
          </div>
          ...
          <div className="space-y-3">
            {resources.map((r) => (
              <div key={r.id} className="flex items-start gap-3 p-3 bg-[var(--c-bg2)]/60 border border-[var(--c-rule)] rounded-xl group">
                {/* Type selector */}
                <select
                  value={r.type}
                  onChange={e => updateResource(r.id, 'type', e.target.value)}
                  className="border border-[var(--c-rule)] rounded-lg px-2 py-2 text-xs bg-[var(--c-surf)] text-[var(--c-ink2)] font-medium focus:outline-none focus:ring-1 focus:ring-brand-500 flex-shrink-0"
                >
                ...
                <div className="flex-1 min-w-0 space-y-2">
                  <input
                    ...
                    className="w-full border border-[var(--c-rule)] rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-brand-500 bg-[var(--c-surf)] transition"
                  />
                  <input
                    type="url"
                    value={r.url}
                    ...
                    placeholder="https://…"
                    className="w-full border border-[var(--c-rule)] rounded-lg px-3 py-2 text-sm font-mono focus:outline-none focus:ring-1 focus:ring-brand-500 bg-[var(--c-surf)] transition"
                  />
                </div>
                ...
                <button
                  ...
                  className="p-1.5 text-[var(--c-ink3)] hover:text-red-500 hover:bg-red-500/10 rounded-lg transition-colors flex-shrink-0 mt-0.5"
                  title="Remove"
                >
                  <Trash2 size={14} />
                </button>
            ...
            <p className="text-xs text-[var(--c-ink3)] pt-1">
      ...
      {isEdit && (
        ...
        >
          ...
          <div className="space-y-2">
            {videos.map((v, idx) => (
              <div key={v.id} className="border border-[var(--c-rule)] rounded-xl overflow-hidden">
                {/* Video row */}
                <div className="flex items-center gap-3 px-4 py-3">
                  <GripVertical size={15} className="text-[var(--c-ink3)] flex-shrink-0 cursor-grab" />
                  <span className="w-6 h-6 rounded-full bg-brand-500/10 text-[11px] font-bold text-brand-600 flex items-center justify-center flex-shrink-0">
                    {idx + 1}
                  </span>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="text-sm font-semibold text-[var(--c-ink)] truncate">{v.title}</p>
                      {v.is_preview && (
                        <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 uppercase tracking-wider flex-shrink-0">
                          Preview
                        </span>
                      )}
                    </div>
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <button
                      ...
                      className={clsx(
                        ...
                        quizVideoId === v.id
                          ...
                          : 'text-brand-600 border-brand-500/30 hover:bg-brand-500/10'
                      )}
                    >
                      Quiz
                    </button>
                    <button
                      ...
                      className="text-xs px-2.5 py-1.5 rounded-lg border border-[var(--c-rule)] text-[var(--c-ink2)] hover:border-brand-300 hover:text-brand-400 font-semibold transition-all"
                    >
                      Edit
                    </button>
                    <button
                      onClick={() => { if (confirm('Delete this video and its Q&A?')) deleteVideo.mutate(v.id); }}
                      className="p-1.5 text-[var(--c-ink3)] hover:text-red-600 hover:bg-red-500/10 rounded-lg transition-colors"
                    >
                      <Trash2 size={14} />
                    </button>
                  </div>
                ...
                {editingVideo?.id === v.id && (
                  <div className="border-t border-[var(--c-rule)] px-4 pb-4 pt-3 bg-[var(--c-bg2)]/40">
                    <VideoFormCard
                      form={editingVideo}
                      setForm={setEditingVideo}
                      onSave={() => saveVideo.mutate(editingVideo)}
                ...
                {/* Transcript manager */}
                {transcriptVideoId === v.id && (
                  <div className="border-t border-indigo-500/25 p-4 bg-indigo-500/10/20">
                ...
                {/* Quiz builder */}
                {quizVideoId === v.id && (
                  <div className="border-t border-[var(--c-rule)] p-4 bg-[var(--c-bg2)]/30">
            ...
            {videos.length === 0 && !editingVideo && (
              <div className="flex flex-col items-center justify-center py-12 text-center">
                <div className="w-10 h-10 bg-[var(--c-bg2)] rounded-xl flex items-center justify-center mb-3">
                  <Video size={18} className="text-[var(--c-ink3)]" />
                </div>
                <p className="text-sm font-medium text-[var(--c-ink2)]">No videos yet</p>
                <p className="text-xs text-[var(--c-ink3)] mt-1">Add videos to this module to get started</p>
              </div>
            )}
          </div>
        </Section>
        ...
        >
          <div className="space-y-2">
            {lessons.map((lesson, idx) => (
              <div key={lesson.id} className="border border-[var(--c-rule)] rounded-xl overflow-hidden">
                {/* Lesson row */}
                <div className="flex items-center gap-3 px-4 py-3">
                  <GripVertical size={15} className="text-[var(--c-ink3)] flex-shrink-0 cursor-grab" />
                  <span className="w-6 h-6 rounded-full bg-amber-500/10 text-[11px] font-bold text-amber-600 flex items-center justify-center flex-shrink-0">
                    {idx + 1}
                  </span>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-[var(--c-ink)] truncate">{lesson.title}</p>
                    <p className="text-xs text-[var(--c-ink3)]">
                ...
                {editingLessonId === lesson.id && (
                  <div className="border-t border-amber-500/25 px-4 pb-4 pt-3 bg-amber-500/10/10">
                    <LessonEditor
                      lesson={lesson}
            ...
            {lessons.length === 0 && (
              <div className="flex flex-col items-center justify-center py-12 text-center">
                <div className="w-10 h-10 bg-[var(--c-bg2)] rounded-xl flex items-center justify-center mb-3">
                  <BookOpen size={18} className="text-[var(--c-ink3)]" />
                </div>
                <p className="text-sm font-medium text-[var(--c-ink2)]">No note lessons yet</p>
                <p className="text-xs text-[var(--c-ink3)] mt-1 mb-3">
                ...
                <button
                  type="button"
                  onClick={() => createLesson.mutate()}
                  className="text-xs font-semibold text-amber-600 hover:text-amber-400 transition-colors"
                >
                  + Add first lesson
                </button>
              </div>
      ...
      {!isEdit && (
        <div className="bg-brand-500/10 border border-brand-500/25 rounded-xl p-4 text-sm text-brand-400">
          <p className="font-semibold mb-0.5">Next steps after creating the module:</p>
          <p className="text-brand-400 text-xs">You'll be able to add videos, quizzes, and more resources once the module is created.</p>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-1.5">
        <label className="text-xs font-semibold text-[var(--c-ink2)]">
          {label} {required && <span className="text-red-400">*</span>}
        </label>
        <div className="flex rounded-lg border border-[var(--c-rule)] overflow-hidden text-[11px] font-semibold">
          {allowRecord && (
            <button
              type="button"
              onClick={() => setMode('record')}
              className={clsx('px-2.5 py-1 flex items-center gap-1 transition-colors',
                mode === 'record' ? 'bg-brand-600 text-white' : 'text-[var(--c-ink2)] hover:bg-[var(--c-bg2)]')}
            >
          ...
          <button
            ...
            onClick={() => setMode('url')}
            className={clsx('px-2.5 py-1 flex items-center gap-1 transition-colors',
              mode === 'url' ? 'bg-brand-600 text-white' : 'text-[var(--c-ink2)] hover:bg-[var(--c-bg2)]')}
          >
          ...
          <button
            ...
            className={clsx('px-2.5 py-1 flex items-center gap-1 transition-colors',
              mode === 'upload' ? 'bg-brand-600 text-white' : 'text-[var(--c-ink2)] hover:bg-[var(--c-bg2)]')}
          >
        ...
        <div>
          ...
          <div
            ...
            className={clsx(
              ...
              dragOver ? 'border-brand-400 bg-brand-500/10' : 'border-[var(--c-rule)] hover:border-brand-300 hover:bg-[var(--c-bg2)]'
            )}
          >
            {uploading ? (
              <div className="flex flex-col items-center gap-2 py-1">
                <div className="w-5 h-5 border-2 border-brand-500 border-t-transparent rounded-full animate-spin" />
                <p className="text-xs text-[var(--c-ink2)]">Uploading to Supabase…</p>
              </div>
            ) : (
              <div className="flex flex-col items-center gap-1.5 py-1">
                <UploadCloud size={20} className="text-[var(--c-ink3)]" />
                <p className="text-xs font-medium text-[var(--c-ink2)]">Drop file here or <span className="text-brand-600">browse</span></p>
                <p className="text-[10px] text-[var(--c-ink3)]">{accept.replace(/,/g, ' ·').replace(/\./g, '')}</p>
              </div>
            )}
          </div>
          ...
          {value && (
            <div className="flex items-center gap-2 mt-2 bg-emerald-500/10 border border-emerald-500/30 rounded-lg px-3 py-1.5">
              <span className="text-[11px] text-emerald-400 truncate flex-1 font-mono">{value}</span>
        ...
        <div>
          {value ? (
            ...
            <button
              ...
              className="w-full flex flex-col items-center gap-2 py-5 border-2 border-dashed border-brand-500/30 rounded-lg bg-brand-500/10/40 hover:bg-brand-500/10 hover:border-brand-300 transition-colors cursor-pointer"
            >
              <div className="w-9 h-9 rounded-xl bg-brand-500/15 flex items-center justify-center">
                <ScreenShare size={17} className="text-brand-600" />
              </div>
              <div className="text-center">
                <p className="text-xs font-semibold text-brand-400">Record your screen</p>
    ...
    <div className="space-y-3">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div className="col-span-1 sm:col-span-2">
          <label className="block text-xs font-semibold text-[var(--c-ink2)] mb-1">Title <span className="text-red-400">*</span></label>
        ...
        <div>
          <label className="block text-xs font-semibold text-[var(--c-ink2)] mb-1">Duration (seconds)</label>
